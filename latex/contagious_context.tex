...
nonlinear P\'olya urn, and a single self-reinforcing injected count exhibits a
\emph{condensation} transition: shrugged off below a trust threshold, locked in above it.
...
(\S\ref{sec:delivery}), and the transmission (\S\ref{sec:worm}).

% ===========================================================================
\section{Model and definitions}\label{sec:model}
% ===========================================================================
We use the architecture of~\cite{shumow2026trust} and adopt its notation. A frozen global
model $g$ over a vocabulary of size $V$ supplies a smoothed predictive distribution
...
\eqref{eq:pstep} to three digits across the reliance sweep. It sits \emph{slightly above} the
static curve at high reliance, and the measured run length \emph{exceeds} the static geometric
...
The canonical exact quine at order $k$ is a de Bruijn cycle $B(2,k)$ of period $2^{k}$ over a
two-symbol alphabet. Sweeping orders $2$--$5$ (Figure~\ref{fig:orders}) gives two facts. First,
...
period needs $p_{\text{step}}^{\,2^{k}}$, so the reproduced-periods-before-slip fall sharply
with $k$ (at $\rho=0.996$: $63,30,15,9$ periods for $k=1,\dots,4$ respectively). Order is a
brittleness dial, not a per-step one.

A clean structural fact underlies why the divisibility question of \S\ref{sec:divis} is even
well-posed.
...
$T=60$) is $3$ counts for the best bridge $\max\,\text{freq}(c)/(n_c+a)$, versus $8$ for the
...
generates ($T$) divided by the string length ($p$). The basic reproduction number is the ratio
of broadcast to threshold,
...
real models is a conjecture, but a sharper one here: the induction (copy) mechanism is precisely
a longest-match cache, so the de Bruijn determinism of \S\ref{sec:orders} and the
