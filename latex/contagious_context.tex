...
\emph{matches} a previous span, not because copying it helps. The toy ships a deliberately
simple surrogate for this as an alternative cache. \texttt{PpmPredictor} is a variable-order online predictor that interpolates
the longest previously-seen suffix down to the global model with absolute discounting---``saw
$A\,B\dots$ now see $A$, predict $B$,'' echoing the copy pattern studied for induction heads~\cite{olsson2022}.
...
its toy-level evidence. The induction (copy) mechanism~\cite{olsson2022} is modeled here by a
